...
\AuthorsBlock
{% No empthy line here
  \Author{Ribas, E. R. L. D.}{1,2}
  \AuthorSep
  \Author{Ramalho, P. L. V. S.}{1}
  \AuthorLastSep
  \Author{Ramalho, J. A. V.}{1}
}
{% No empthy line here
  \Affil{1}{Centro Universitário Dom Helder, Curso de Ciência da Computação, Belo Horizonte, Brazil}
  \Affil{2}{Federal Center of Technological Education of Minas Gerais, Belo Horizonte, Brazil}
}
...
%%%%%%%%%% Abstract %%%%%%%%%%
\abstract{
Este trabalho apresenta a implementação e a análise experimental de algoritmos de caminhos mínimos em grafos direcionados, com foco nos algoritmos clássicos de Dijkstra e Bellman-Ford e no estudo de conceitos introduzidos pelo artigo \textit{Breaking the Sorting Barrier for Directed Single-Source Shortest Paths} (Duan et al., 2025). Inicialmente, foi realizada uma comparação de desempenho entre Dijkstra e Bellman-Ford em grafos aleatórios esparsos e densos. Os resultados confirmaram o comportamento previsto pela teoria de complexidade, evidenciando a superioridade do algoritmo de Dijkstra em todos os cenários avaliados, especialmente em grafos densos. Em seguida, foi desenvolvida e avaliada uma implementação parcial baseada no mecanismo de poda de pivôs descrito pelo Lema 3.2 do artigo estudado. Os experimentos investigaram o impacto do parâmetro de profundidade (k) sobre a redução do conjunto de pivôs e sobre o tempo total de execução. Os resultados demonstraram que a implementação preservou as restrições matemáticas estabelecidas pelo lema, porém a poda efetiva ocorreu apenas para valores elevados de (k). Para os demais cenários, o mecanismo de segurança foi frequentemente acionado, limitando a redução da fronteira e resultando em um custo computacional adicional superior aos benefícios obtidos. Em todas as instâncias avaliadas, a abordagem baseada em pivôs apresentou tempos de execução superiores aos do algoritmo de Dijkstra executado diretamente. Os resultados obtidos contribuem para a compreensão prática dos algoritmos clássicos de caminhos mínimos e dos mecanismos teóricos propostos em pesquisas recentes sobre o problema de caminho mínimo de fonte única.

\textbf{Palavras-chave:} caminhos mínimos; algoritmos em grafos; Dijkstra; Bellman-Ford; BMSSP; análise de desempenho.
}
...
Um dos problemas mais clássicos e fundamentais na teoria de grafos é o problema do caminho mínimo ou \textit{Single Source Shortest Path (SSSP)}, que consiste em encontrar uma otimização de um caminho entre dois objetos em um grafo, em que esse caminho pode ser criterizado por um peso, como distância, tempo, custo, etc.. Existem diversos algoritmos para modelar e encontrar soluções para o problema do caminho mínimo, dentre as principais estão os algoritmos de Dijkstra, Bellman-Ford, A* e mais recentemente, o algoritmo de Proposto por \citep{breakingTheSortingBarrier} chamado de \textit{Bounded Multi-Source Shortest Path (BMSSP)} que é uma adaptação do algoritmo clássico de Dijkstra que promete uma melhoria de desempenho temporal para um caso espécifico de grafos.

Este trabalho tem como objetivo realizar um análise experimental dos algoritmos de Dijkstra, Bellman-Ford e da Sub-rotina FindPivots, que faz parte do núcleo da contribuição de \citep{breakingTheSortingBarrier}, comparando suas eficiências a partir de um \textit{Benchmark} experimental. Através de uma série de experimentos, buscamos entender as vantagens e limitações de cada algoritmo, bem como identificar cenários em que um pode ser preferível ao outro.
...
O algoritmo prosposto por Edsger Dijkstra em 1959, \citep{dijkstra1959note} permanece invicto até a os dias atuais, quando o assunto é algoritmos de SSSP para pesos não negativos. Em paralelo, o Algoritmo proposto por \citep{ford}, conhecido com Algoritmo de Bellman-Ford é a solução clássica que permite a presença de arestas com pesos negativos, embora seja significativamente mais lento que o Dijkstra.

Contrastando com a abordagem gulosa (\textit{greedy}) do Dijkstra, a implementação do algoritmo de Bellman-Ford adota o princípio de programação dinâmica. A sua arquitetura prescinde de estruturas de dados complexas para ordenação, como filas de prioridade (e.g. \textit{Min Heap}), iterando sistematicamente sobre todas as arestas do grafo e aplicando o processo de relaxamento até $|V| - 1$ vezes. 

Por sua vez, o algoritmo de \citep{breakingTheSortingBarrier} propõe uma abordagem híbrida, combinando a eficiência do Dijkstra com uma estratégia \textit{Divide and Conquer}, que consiste numa poda de fronteira baseada em pivôs, que visa reduzir o número de fontes múltiplas a serem processadas, mitigando o impacto temporal do recálculo isolado. O cerne dessa otimização reside na sub-rotina \textit{FindPivots}, que implementa o Lema 3.2 do \citep{breakingTheSortingBarrier}, um mecanismo de busca local que limita a expansão da fronteira $S$ a um subconjunto estrito de pivôs $P$, cuja cardinalidade é teoricamente garantida por $|P| \le \frac{|W|}{k}$, onde $W$ é o conjunto expandido e $k$ é um parâmetro de profundidade.

\section{Experimentos}

O presente trabalho, alinhado às diretrizes de \citep{TP01}, propõe a implementação e a análise comparativa de desempenho dos algoritmos clássicos para o problema de Caminhos Mínimos de Fonte Única ou \textit{Single Source Shortest Path (SSSP)}, notadamente Bellman-Ford e Dijkstra. Além de estabelecer este \textit{benchmark} referencial, o estudo apresenta uma implementação e uma análise empírica crítica da sub-rotina \textit{Find Pivots} (Lema 3.2), que constitui o núcleo da recente contribuição teórica de \cite{breakingTheSortingBarrier}.

Vale ressaltar que o escopo deste trabalho é estritamente experimental, focando na implementação e na avaliação de desempenho dos algoritmos mencionados e não representa uma implementação integral do algoritimo BMSSP proposto por \cite{breakingTheSortingBarrier}. A análise teórica detalhada, incluindo a prova formal do Lema 3.2 e a demonstração da complexidade assintótica do BMSSP, está além do escopo desse relatório e é recomendada para leitura direta na fonte original, e fica como sugestão para trabalhos futuros.

\subsection{Ambiente Experimental}

Todas as implementações do \textit{benchmark} foram integralmente desenvolvidos na linguagem Python (versão 3.14.3), escolhida por sua simplicidade e alta velocidade de desenvolvimento, e estão disponibilizadas em \citep{ribas2026sssp}.
Todos os testes foram realizados em uma máquina com as seguintes especificações:
\begin{table}[h]
\centering
\caption{Ambiente de Teste e Versões. Fonte: Próprio autor.}\label{tab:ambiente_teste}
\begin{tabular}{l l}
\hline
...
\hline
Processador & Intel(R) Core(TM) i3-4010U CPU @ 1.70GHz \textit{Cores} \\
Memória RAM & 16 GB \\
...
\subsubsection{Decisões de Projeto e Desafios Encontrados}

Inicialmente foi adotada uma abordagem com matrizes de adjacencia para a geração dos grafos por meio do algoritmo parametrizável para geração de grafos aleatórios, mas com o crescimento do número de vértices, a complexidade espacial de $O(V^2)$ tornou-se proibitiva, especialmente para grafos densos, o tempo para leitura e escrita dos dados se tornou inviável (cerca de 5-10 minutos para grafos com 10.000 vértices). Para contornar esse desafio, a implementação foi adaptada para utilizar listas de adjacência, que oferecem uma representação mais compacta e eficiente para grafos esparsos, reduzindo a complexidade espacial para $O(V + E)$.

O algoritmo \textit{get\_neighbors} na implementação inicial tinha um tempo de execução $O(V)$ para cada vértice analisado, criando um \textit{overhead} significativo. Já na implementação final com listas de adjacência, o tempo de execução para obter vizinhos de um grafo se torna $O(1)$ ou $O(k)$ para vértices de $k$ vizinhos.

Além disso, para evitar a presença de arestas paralelas e loops, uma estrutura de dados \textit{seen\_nodes} foi introduzida durante a geração dos grafos, garantindo a unicidade das arestas e a integridade estrutural do grafo. Apesar de não ter funcionado inicialmente, o problema foi identificado e corrigido, sua causa era que para grafos com o número de arestas maior que o máximo teórico para um grafo simples $(E > \frac{N(N-1)}{2})$, o algoritmo de geração entrava em um loop infinito tentando gerar arestas únicas, a solução foi limitar o número de arestas para o máximo teórico permitido para um grafo simples.

Ademais, durante os \textit{batches} de \textit{benchmarking}, foi necessário introduzir uma lógica de \textit{caching}, para evitar que os grafos fossem gerados todas as vezes que o algoritmo fosse rodado, economizando uma quantidade siginificativa de tempo.

A implementação de ambos os algoritmos exigiu uma série de decisões de projeto para garantir a eficiência e a correção. Para o algoritmo de Dijkstra, a escolha da estrutura de dados para a fila de prioridade foi crucial, optando-se por um \textit{Binary Heap (heappq)} para equilibrar a complexidade de inserção e extração. Já para o Bellman-Ford, a implementação do mecanismo de \textit{Early Stopping} foi fundamental para evitar iterações desnecessárias em casos onde os caminhos mínimos já haviam convergido.
...
\subsubsection{Discussão e Resultados}
...
\subsection{Discussão e Resultados}
...
Os resultados obtidos neste trabalho permitem analisar tanto o comportamento dos algoritmos clássicos de caminhos mínimos quanto a aplicação prática da estratégia de poda de pivôs baseada no Lema 3.2 do artigo \textit{Breaking the Sorting Barrier for Directed Single-Source Shortest Paths}.

No primeiro experimento, os resultados confirmaram integralmente o comportamento previsto pela análise de complexidade dos algoritmos clássicos. Em todos os cenários avaliados, o algoritmo de Dijkstra apresentou tempos de execução inferiores aos observados para o Bellman-Ford. Essa diferença foi observada tanto em grafos esparsos quanto em grafos densos, tornando-se mais expressiva à medida que o número de arestas aumentava. Enquanto o Dijkstra manteve crescimento controlado dos tempos de execução, o Bellman-Ford apresentou aumento significativamente mais acentuado, refletindo diretamente a diferença entre suas complexidades teóricas.

O segundo experimento teve como foco a avaliação da estratégia de poda de pivôs proposta pelo Lema 3.2. Os resultados demonstraram inicialmente que a implementação desenvolvida preserva a corretude matemática do método. Em todas as execuções realizadas, a restrição teórica que limita o número de pivôs foi respeitada, validando a implementação da lógica de extração e filtragem descrita pelo lema.

Entretanto, a análise experimental mostrou que a poda efetiva raramente ocorreu nos cenários avaliados. Para os valores de profundidade considerados no cenário principal $((k \le 16))$, o conjunto de pivôs permaneceu praticamente idêntico ao tamanho da fronteira inicial. Os dados mostram que o gatilho de segurança responsável por interromper a expansão da busca foi acionado repetidamente, fazendo com que a rotina retornasse a fronteira quase sem redução. Como consequência, o algoritmo realizou todo o trabalho associado à expansão local, construção das estruturas auxiliares e análise da floresta de busca sem obter uma diminuição significativa do número de pivôs.

Esse comportamento refletiu-se diretamente no desempenho observado. Em todos os valores testados para (N=1000) e $(k \le 16)$, a abordagem baseada em pivôs apresentou penalidades superiores a 29 vezes o tempo do Dijkstra clássico, atingindo mais de 38 vezes para determinados valores de (k). Assim, o custo adicional introduzido pelo processo de poda mostrou-se significativamente superior ao benefício efetivamente obtido.

A análise complementar realizada com valores maiores de (k) permitiu identificar quando a poda passa a ocorrer de forma mais intensa. Os resultados mostram que reduções expressivas no número de pivôs surgem apenas para valores elevados de profundidade, como (k=32), (64) ou (128), dependendo do tamanho da instância. Nesses casos, a quantidade de pivôs diminui substancialmente e o tempo total da abordagem também é reduzido. Contudo, observa-se simultaneamente que o conjunto expandido (W) já alcança centenas ou milhares de vértices, indicando que uma parcela significativa do grafo foi explorada antes da realização da poda.

Os testes de escalabilidade reforçam essa tendência. À medida que o número de vértices cresce, a diferença entre o Dijkstra clássico e a implementação baseada em pivôs torna-se ainda mais evidente. Para instâncias com 5.000 e 10.000 vértices, a penalidade permaneceu elevada mesmo para valores relativamente altos de (k), alcançando dezenas de vezes o tempo necessário para executar o Dijkstra tradicional. Embora a redução do conjunto de pivôs eventualmente ocorra, o custo acumulado das etapas anteriores continua predominante.

Portanto, considerando exclusivamente os cenários experimentais avaliados neste trabalho, os resultados indicam que a implementação baseada no Lema 3.2 preserva as garantias matemáticas descritas pelo artigo, porém não produziu ganhos de desempenho em relação ao algoritmo de Dijkstra. A estratégia de poda mostrou-se limitada pela ativação frequente do mecanismo de segurança e pelo elevado custo computacional associado às etapas de pré-processamento. Dessa forma, para todas as instâncias analisadas, o Dijkstra clássico permaneceu como a alternativa mais eficiente em termos de tempo de execução.


\section{Conclusão}

Este trabalho teve como objetivo implementar e analisar algoritmos de caminhos mínimos em grafos direcionados, explorando tanto abordagens clássicas quanto conceitos introduzidos pelo artigo \textit{Breaking the Sorting Barrier for Directed Single-Source Shortest Paths} (Duan et al., 2025). Para isso, foram desenvolvidas implementações dos algoritmos de Dijkstra e Bellman-Ford, além de uma implementação parcial da estratégia de otimização de fronteira baseada no procedimento \textit{FindPivots} e no Lema 3.2 apresentado pelos autores.

A comparação entre os algoritmos clássicos confirmou experimentalmente os resultados previstos pela teoria, evidenciando o melhor desempenho do algoritmo de Dijkstra em todos os cenários avaliados. Os experimentos também permitiram validar o funcionamento da implementação desenvolvida para a estratégia de poda de pivôs, verificando o atendimento às restrições matemáticas estabelecidas pelo lema estudado.

Embora a abordagem baseada em pivôs não tenha produzido ganhos de desempenho nos cenários experimentais considerados, sua implementação possibilitou compreender, na prática, os mecanismos que compõem uma das contribuições centrais do artigo analisado. Dessa forma, o trabalho cumpriu seu objetivo de investigar tanto os fundamentos clássicos do problema de caminhos mínimos quanto técnicas mais recentes propostas na literatura.

Além dos resultados obtidos, o projeto proporcionou uma experiência prática na implementação, instrumentação e avaliação experimental de algoritmos em grafos, permitindo relacionar conceitos teóricos de análise de complexidade com medições reais de desempenho.

Como continuidade deste estudo, trabalhos futuros podem explorar diferentes classes de grafos, analisar o comportamento da estratégia de poda em outras topologias e incorporar componentes adicionais descritos no artigo original, ampliando a investigação sobre a aplicabilidade prática das técnicas propostas pelos autores.
