...
class Graph:
    ...
    def find_pivots(
        ...
        for _ in range(k):
            ...
            for u in current_layer:
                for v, weight in self.get_neighbors(u):
                    new_dist = d_hat.get(u, float("inf")) + weight
        ...
        while stack:
            u = stack.pop()
            r = u if u in roots else root_of[u]
            root_of[u] = r
            tree_size[r] += 1
        ...
        S_set = set(S)
        P: set[int] = {r for r in roots if r in S_set and tree_size.get(r, 0) >= k}
    ...
    def partial_dijkstra(
        ...
        b = min(dist[s] for s in S) if S else float("inf")
        return dist, b, S
    ...
    def optimized_dijkstra_with_pivots(
        ...
        combined_dist: dict[int, float] = {
            node: d_hat.get(node, float("inf")) for node in range(self.nodes)
        }
        ...
        return combined_dist, metrics, end - start
...
class BenchmarkRunner:
    ...
    def _run_experiment_2_sensitivity(self) -> list[dict]:
        ...
        for n in n_values:
            ...
            print(f"\nEvaluating Sensitivity for N={n}...")
